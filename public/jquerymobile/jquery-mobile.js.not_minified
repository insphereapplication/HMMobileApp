...
function switchPage(page, transition, isTracked)
{
  $.mobile.changePage(page, transition, false, isTracked);
}
...
/* exposed $.mobile methods  */
  ...
  // changepage function
  $.mobile.changePage = function( to, transition, back, changeHash, useDOM){
    ...
    var toIsArray = $.type(to) === "array",
      ...
      //ALLEN Remove check for previous url = new url
      duplicateCachedPage = null;
      //back = (back !== undefined) ? back : ( urlStack.length > 1 && urlStack[ urlStack.length - 2 ].url === url );

    //ALLEN remove check if url == same as current page
    ...
    //ALLEN fix to allow for a 'back' transition
/*
...
// *** END MOD ***    
    ...
    function transitionPages() {
      ...
      function loadComplete(){
        ...
        //ALLEN remove everything else from DOM
        if (changeHash != false) {
      ...
      // ALLEN Disable transitions on bb6.
      if(transition && (transition !== 'none') && (navigator.platform != 'BlackBerry')){
    ...
    //ALLEN modify to prevent loading pages from DOM
/*    
...
//ALLEN **** BEGIN MOD ****
    // find the "to" page, either locally existing in the dom or by creating it through ajax
    if ( to.length && !isFormRequest && useDOM) {
      if( fileUrl && base ){
        base.set( fileUrl );
      }
      enhancePage();
...
//ALLEN MOD selectmenu plugin upgraded to latest from github.
/*
...
$.widget( "mobile.selectmenu", $.mobile.widget, {
  ...
  open: function(){
    ...
    if( dialogUsed || menuHeight > screenHeight - 80 || !$.support.scrollTop ){
      ...
      //ALLEN port new jqm code to work with old code, added useDOM param
      //$.mobile.changePage(self.menuPage, 'pop', false, true);
      $.mobile.changePage(self.menuPage, 'pop', false, false, true);
    }
    else {
  ...
  close: function(){
    ...
    if(self.menuType == "page"){
      ...
      $.mobile.changePage([self.menuPage,self.thisPage], 'pop', true, false, true);
...
$.widget( "mobile.dialog", $.mobile.widget, {
  ...
  close: function(){
    //ALLEN added useDOM param
    $.mobile.changePage([this.element, $.mobile.activePage], undefined, true, true, true );
  }
});
})( jQuery );
